...
GLSLProgram::GLSLProgram(const pugi::xml_node& node) {
	if (!loadXMLSettings(node)) {
		nge_log_error("Could not load xml settings for " + boost::lexical_cast<string>(node.name()));
	}
}
...
bool GLSLProgram::loadXMLSettings(const pugi::xml_node& node) {
	...
	if (name.length() == 0) {
		nge_log_error("Shader attribute name is not valid");
		return false;
	}
	...
	if (vertexShaderNode.empty() || vertexShaderNode.child("Raw").empty()) {
		nge_log_error("<VertexShader> node not found in " + name);
		return false;
	} else {
	...
	if (fragmentShaderNode.empty() || fragmentShaderNode.child("Raw").empty()) {
		nge_log_error("<FragmentShader> node not found in " + name);
		return false;
	} else {
		// TODO: Odczytanie zmiennych uniform
...
bool GLSLProgram::initialize(bool bindAttribs) {
	...
	if (programId == 0) {
		nge_log_error("GLSL Shader --> Could not create program");
		return false;
	}

	vertexShader.id = glCreateShader(GL_VERTEX_SHADER);
	if (vertexShader.id == 0) {
		nge_log_error("GLSL Shader --> Could not create vertex shader");
		return false;
	}

	fragmentShader.id = glCreateShader(GL_FRAGMENT_SHADER);
	if (fragmentShader.id == 0) {
		nge_log_error("GLSL Shader --> Could not create fragment shader");
		return false;
	}
	...
	if (vertexShader.source.empty() || fragmentShader.source.empty()) {
		nge_log_error("GLSL Shader --> Shader source code could not be empty");
		return false;
	}
	...
	if (!compileShader(vertexShader) || !compileShader(fragmentShader)) {
		nge_log_error("GLSL Shader --> Could not compile the shaders, they are invalid");
		return false;
	}
...
string GLSLProgram::readFile(const string& filename) {
	...
	if (!fileIn.good()) {
		nge_log_error("Could not load shader: " + nge_to_string(filename));
		return string();
	}
...
bool GLSLProgram::compileShader(const GLSLShader& shader) {
	...
	if (!result) {
		nge_log_error("Could not compile shader: " + nge_to_string(shader.id));
