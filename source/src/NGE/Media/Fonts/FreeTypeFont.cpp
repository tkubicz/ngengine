...
bool FreeTypeFont::Initialize() {
    ...
    if (FT_Init_FreeType(&library)) {
        log_error("Could not initialise the freetype library");
        return false;
    }

    FT_Face fontInfo;
    if (FT_New_Face(library, fontName.c_str(), 0, &fontInfo)) {
        log_error("Could not load the specified font");
        return false;
    }
    ...
    if (!shader) {
        log_error("Font shader is not set");
        return false;
    }
...
bool FreeTypeFont::GenerateAtlasTexture(FT_Face& fontInfo) {
    ...
    for (int i = 32; i < 128; ++i) {
        if (FT_Load_Char(fontInfo, i, FT_LOAD_RENDER)) {
            log_error("Could not load character: '{}'", i);
            continue;
        }
    ...
    log_info("Generated a " + to_string(atlasWidth) + " x "
...
bool FreeTypeFont::LoadXMLSettings(const pugi::xml_node& node, const std::string& path) {
    if (std::string(node.name()) != "FreeTypeFont") {
        log_error("Need a font node");
        return false;
    }

    if (path.length() == 0) {
        log_error("Need a path");
        return false;
    }

    if (node.attribute("fileName").empty()) {
        log_error("Need a fileName attribute");
        return false;
    } else
