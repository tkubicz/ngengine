...
bool FontManager::loadFont(const pugi::xml_node& node) {
	if (node.empty() || std::string(node.name()) != "FreeTypeFont") {
		nge_log_error("FontManager --> Need FreeTypeFont node");
		return false;
	}

	std::string name = node.attribute("name").as_string();
	if (name.length() == 0) {
		nge_log_error("FontManager --> Could not find name attribute");
		return false;
	}
#ifdef NGE_USE_FREETYPE  
	...
	if (it != fonts.end())
		...
		for (std::vector<std::string>::iterator i = paths.begin(); i != paths.end(); ++i) {
			if (font->LoadXMLSettings(node, *i)) {
				...
				nge_log_info("FontManager --> New font added: " + name + " (" + font->GetFontName() + ")");
				return true;
			}
		}

		delete font;
		nge_log_error(std::string("FontManager --> Could not load font: ") + name);
		return false;
	}
#else
	return false;
...
FreeTypeFont* FontManager::GetFont(const std::string& name) {
	...
	nge_log_error("FontManager --> Could not find font: " + name);
	return NULL;
}
