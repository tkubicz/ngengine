...
bool GUIPanel::LoadXMLSettings(const pugi::xml_node& node)
{
    ...
    {
        Tools::Logger::WriteErrorLog("Need a Panel node in the xml file");
        return false;
    }
...
bool GUIPanel::AddWidget(GUIRectangle* widget)
{
    if (widget)
    {
        for (size_t i = 0; i < elements.size(); ++i)
        {
            ...
            {
                Tools::Logger::WriteErrorLog("Panel already has a child with callbackString = " + widget->GetCallbackString());
                return false;
            }
        }
