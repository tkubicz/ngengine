...
void ObjFileParser::GetMaterialDesc() {
    ...
    if (it == model->materialMap.end()) {
        ...
        log_error("Failed to locate material '{}', skipping", name);
    } else {
...
void ObjFileParser::GetMaterialLib() {
    ...
    if (!file) {
        log_error("Failed to open '{}'", matName);
        dataIt = SkipLine<DataArrayIt>(dataIt, dataItEnd, line);
        return;
    }
...
void ObjFileParser::GetNewMaterial() {
    ...
    if (it == model->materialMap.end()) {
        ...
        log_warn("Unsupported material requested: '{}", mat);
        model->currentMaterial = model->defaultMaterial;
    } else {
        // Set new material
        if (NeedsNewMesh(mat)) {
...
void ObjFileParser::CreateMesh() {
    ...
    if (nullptr != model->current) {
        ...
        log_error("No object detected to attach a new mesh instance.");
    }
}
...
void ObjFileParser::ReportErrorTokenInFace() {
    dataIt = SkipLine<DataArrayIt>(dataIt, dataItEnd, line);
    log_error("Not supported token in face description detected.");
}
