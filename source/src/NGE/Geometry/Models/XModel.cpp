...
bool XModel::Parse() {
    ...
    if (strncmp(p, "xof ", 4) != 0) {
        log_error("Header mismatch. File is not an XFile");
        return false;
    }
    ...
    if (strncmp(p + 8, "txt ", 4) == 0)
        ...
        log_error("Unsupported xfile format");
        return false;
    }
    ...
    if (binaryFloatSize != 32 && binaryFloatSize != 64) {
        log_error("Unknown float size specified in xfile header");
        return false;
    }
    ...
    if (compressed) {
        ...
        while (p1 + 3 < end) {
            ...
            if (ofs >= MSZIP_BLOCK) {
                log_error("Invalid offset to next MSZIP compressed block");
                return false;
            }
            ...
            if (magic != MSZIP_MAGIC) {
                log_error("Unsupported compressed format, expected MSZIP header");
                return false;
            }
        ...
        while (p + 3 < end) {
            ...
            if (ret != Z_OK && ret != Z_STREAM_END) {
                log_error("Failed to decompress MSZIP-compressed data");
                return false;
            }
        ...
        log_debug("Successfully decompressed MSZIP-compressed file");
    } else {
        // Start reading here.
        ReadUntilEndOfLine();
    }
...
void XModel::ParseFile() {
    ...
    while (running) {
        ...
        if (objectName == "template")
            ...
            // Whatever?
            log_info("Found \'}\' in dataObject");
        } else {
            // Unknown format
            log_info("Unknown data object in x file: '{}'", objectName);
...
void XModel::ParseDataObjectTemplate() {
    ...
    while (running) {
        ...
        if (s.length() == 0)
            log_error("Unexcepted end of file reached while parsing template definition.");
    }
}
...
void XModel::ParseDataObjectMaterial(Material* material) {
    ...
    if (matName.empty()) {
        ...
        matName = fmt::format("material{}", lineNumber);
    }
    material->name = matName;
